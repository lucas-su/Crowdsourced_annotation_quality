...
class inspect():
    ...
    def __call__(self, *args, **kwargs):
        sum_fo = sum([self.sum(nid) for nid in range(self.dup)])
        ...
        for gt in gts:
            ...
            for cnt in cnts:
                print(f'{self.count_more_than_one_FO(gt, cnt)} times >={cnt} FO annotators with n(GT) '
        ...
        mean2 = np.mean([pcs_1[key] for key in pcs_1.keys()])
    ...
    def count_more_than_one_FO(self, GT_val, n):
        ...
        combs = list(it.combinations(np.unique(np.arange(n)), n))
        counts = [self.ann.loc[(self.ann['GT'] == GT_val).values & self.fos(comb), 'GT'].count() for comb in combs]
        return sum(counts)
...
if __name__ == "__main__":
    ...
    # cnts = range(1, 5)
    # gts = range(car)
    inspection_data = pandas.DataFrame(columns=['dup', 'p_fo', 'car', 'mode', 'per_fo_1', 'per_fo_2', 'sumfo', 'obj'])

    mode = 'uniform'
    cars = list(range(2, 10))
    dups = [3, 5, 7, 9]
